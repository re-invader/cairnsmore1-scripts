...
void display(char *buf)
{
	...
	char *accepted, *rejected, *megahash, *errors, *utility;
	int itemcount;
		...
		if (*buf) {
			...
			while (item != NULL) {
				...
				if (*item) {
					...
					if (eq != NULL) {
						if (strcmp(item, "Accepted") == 0)
						    accepted = eq;
						if (strcmp(item, "Rejected") == 0)
						    rejected = eq;
						if (strcmp(item, "MHS av") == 0)
						    megahash = eq;
						if (strcmp(item, "Hardware Errors") == 0)
						    errors = eq;
						if (strcmp(item, "Utility") == 0)
						    utility = eq;
					}
				}
	...
	printf("%s-%s-%s-%s-%s\n", megahash, accepted, rejected, errors, utility);
}
